...
- **Jest via two projects** (`package.json`'s `jest.projects`), not one config, because they need genuinely different environments:
  ...
  - **`integration`** (`__tests__/integration/**`, run via `npm run test:integration`) — plain Node environment with a dedicated `babel.integration.config.js` (deliberately *not* `babel-preset-expo`, which rewrites `process.env.EXPO_PUBLIC_*` into an `expo/virtual/env` import that only resolves inside Metro and breaks under Jest). Hits the real Supabase project over the network — real `fetch`, no mocks. Needs `ws` supplied as the Realtime transport (`realtime: { transport: WebSocket as WebSocketLikeConstructor }` from `@supabase/realtime-js`) purely so `createClient()` doesn't throw on Node 20 (no native `WebSocket`) — Realtime itself is never used.
  - `npm run test:all` runs both. CI (`.github/workflows/test.yml`) runs `unit` then `integration` on every push/PR to `main`.
- **RLS/security suite** (`__tests__/integration/rls.test.ts`): no Supabase branching available (requires a Pro-plan upgrade — a real recurring-cost decision that was surfaced to and deferred by the user, not attempted), so it runs against the live project directly with disposable-but-fixed fixture accounts. See the file's own header comment for the full reasoning; the short version:
  - **Fixed fixture accounts, not per-run signups.** Supabase's auth email rate limit fires on every `signUp()` call — even calls immediately bypassed via the `dev_confirm_user_email` RPC still trigger it — and has been observed as tight as ~1 signup per ~60–90 minutes on this project's free tier. A suite that signs up fresh users every run is not CI-safe. The three fixtures (`bandmate.rls.fixture.{a,b,c}@gmail.com`) self-bootstrap on first run (sign in; on failure, sign up once) and are reused (sign in only) on every run after.
  - **Fixture emails must be on a domain with valid MX records.** GoTrue rejects signups on domains it can't resolve mail for — fake/reserved TLDs (`.dev`, `example.com`) get `email address invalid`. `gmail.com` works fine without needing a real mailbox behind it (no email ever actually needs to arrive, since `dev_confirm_user_email` bypasses confirmation).
...
- **Critical-path suite** (`__tests__/integration/signup-onboarding.test.ts`): tests the signup flow itself, so — unlike the RLS suite — it inherently cannot be redesigned around fixed fixtures; it calls `signUp()` for real, once per run. This means it **will** hit the same auth rate limit if run too frequently (including via CI on frequent pushes to `main`) — an accepted, disclosed limitation of testing a live signup flow, not a bug. It also can't clean up its own `auth.users` row afterward (no service-role key available client-side — see below), so test accounts (`bandmate.critpath.*@gmail.com`) accumulate and need periodic manual deletion.
- **No service-role key available anywhere** — the Supabase MCP server only exposes anon/publishable keys (`get_publishable_keys`), by design. This is why: (a) fixture bootstrap uses real `signUp()`/`dev_confirm_user_email` rather than an admin-created user, (b) test cleanup that needs elevated privileges (deleting `auth.users` rows) is done manually via the MCP `execute_sql` tool in an interactive session, not automatically in CI. If a service-role key is ever added as a GitHub Actions secret, the critical-path test could switch to `supabase.auth.admin.deleteUser()` in an `afterAll` for real per-run cleanup.
- **CI secrets required**: the `integration` GitHub Actions job needs `EXPO_PUBLIC_SUPABASE_URL` and `EXPO_PUBLIC_SUPABASE_ANON_KEY` set as repo secrets (Settings → Secrets and variables → Actions) — same values as the local `.env`, which is gitignored and never committed.
...
- **No Supabase branching**: requires a Pro-plan upgrade (real recurring cost); surfaced to the user, deferred. Integration tests run against the live project instead, with the fixture/cleanup discipline described under Testing above. Revisit if the plan is ever upgraded — real branch-per-run testing would be strictly better once available.
...
- **Integration test accounts accumulate**: `bandmate.rls.fixture.{a,b,c}@gmail.com` are permanent fixtures (intentional). `bandmate.critpath.*@gmail.com` accounts from the critical-path suite are not — each run adds one, undeleted (no service-role key to clean up with). Periodic manual cleanup via SQL (`delete from auth.users where email like 'bandmate.critpath.%'`) is reasonable maintenance, not urgent.
